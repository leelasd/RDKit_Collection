...
def set_pH(in_smi):
    ...
    out_f.close()
    my_list = [babel_path, "-ismi", in_f.name, "-osmi", out_f.name, "-p", "7.4"]
...
def add_new_comp(mol, option=None):
...
# Now attribute all this meta-deta to the compound object
    ...
    try:
        ...
        newer_comp = Compound.objects.get(inchi=inchi)
        newer_comp.num_added += 1
        newer_comp.save(update_fields=["num_added"])
        if option is None:
