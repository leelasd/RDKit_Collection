...
from IOhandle.functions import centre_of_mass_from_SD_block,set_pH,make_smarts_from_frag
...
def store_mmp(outlines):
    ...
    for line in outlines:
        ...
        if(len(core) == 0):
            ...
            make_new_mmp(context, my_id, core, my_dict["sidechains"][1],1,core)
            context = side_chains[1]
            core = side_chains[0]
            #print "CHAIN",side_chains[1],side_chains[0]
            make_new_mmp(context, my_id, core,my_dict["sidechains"][0],1,core)
    #double or triple cut
        else:
            attachments = core.count('*')
            #add the context with id to index
            make_new_mmp(context, my_id, core,my_dict["core"],attachments,core)


def make_mol_mmp(mol, myid=None, target_id=None):
...
def find_mmps(rdmols, target_id):
    ...
    for i, mol in enumerate(rdmols):
        ...
        if mol is None:
            ...
            continue
        make_mol_mmp(mymol, val, target_id)
    old = 100
    sys.stdout.write("\r%d%% complete..." % old)
    sys.stdout.flush()
    print "Completed"
...
def make_list_mmps(target_id, option=None, max_size=10, ratio=3, use_ratio=False):
    ...
    for key in myKeys:
        ...
        # Or if we're dealing with 2D activity molecules
        elif option is "ACT":
...
def make_mmp_database(target_id, option=None):
    ...
    if option is "TWOD":
        ...
        for act in mol_list:
            # Trusting that the input SD's are set with the appropriate pH
...
def act_mmp_3d(out_mmps, target_id):
    ...
    # If its a single cut
        ...
        out_confs = generate_conformations(mol2, core_mol, num_confs=100, num_fails=200, max_iters=200, ff="MMFF")
        if out_confs is None:
            continue
        my_mols = [Chem.MolFromMolBlock(x) for x in out_confs]
        mols = []
        for conf in my_mols:
    ...
    for i, myfrag in enumerate(new_frags):
        ...
        for ph_pt in over_mol_ph4:
            ...
            for ph_pt2 in mol_ph4:
                ...
                # Arom5 != Arom6
                if ph_pt.uniq_id == ph_pt2.uniq_id:
...
def find_frag_com():
    ...
    frags = MMPFrag.objects.exclude(smistore__contains="[*:2]").filter(mol_id__isnull=False, core_size__lte=10)
    ...
    for j, my_list in enumerate(lists):
        ...
        for k, item in enumerate(my_list):
            ...
            if (item[0], item[1]) != old_list:
                ...
                act_points = ActivityPoint.objects.filter(cmpd_id=new_com.xtal_mol.cmpd_id, target_id=new_com.xtal_mol.prot_id.target_id, source__in=ast.literal_eval(activity_type)).order_by("activity")
                if len(act_points) > 1:
                ...
                act_points = ActivityPoint.objects.filter(cmpd_id=new_com.chembl_mol.cmpd_id, target_id=new_com.chembl_mol.prot_id.target_id, source__in=ast.literal_eval(activity_type)).order_by("activity")
                if len(act_points) > 1:
                    # If the user has added multiple points. The the biggest one is taken
