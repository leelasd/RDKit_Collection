...
from Pharmacophore.models import UniquePharma,PharmaPoint
...
class ActPharmaPoint(models.Model):
    ...
    # Number using 3.5
    num_diff_10 = models.IntegerField(null=True)
    # Is it in this point?
    in_diff_10 = models.BooleanField(default=False)
    # Number using 3.5
    num_diff_15 = models.IntegerField(null=True)
    # Is it in this point?
    in_diff_15 = models.BooleanField(default=False)
    # Number of points using 2.5,
    num_diff_25 = models.IntegerField(null=True)
    # Is it in this point?
    in_diff_25 = models.BooleanField(default=False)
    # Number using 3.0
    num_diff_30 = models.IntegerField(null=True)
    # Is it in this point?
    in_diff_30 = models.BooleanField(default=False)
    # Number using 3.5
    num_diff_35 = models.IntegerField(null=True)
    # Is it in this point?
    in_diff_35 = models.BooleanField(default=False)
    # Log difference in activity
    act_change = models.FloatField()
...
def make_new_mmp(context, my_id, core, rdmol, attachments, smi):
    ...
    Returns None"""
    # Make the MMP context
    new_mmp = MMP.objects.get_or_create(context=context.replace("1*", "*:1"))[0]
    ...
    if  my_type == "mol":
        ...
        Chem.Mol.SetProp(rdmol, "_Name", str(my_mol.target_id.title))
    ...
    # Get the SDF info
    new_frag.sdf_info = Chem.MolToMolBlock(rdmol)
    # Get the link to the MMP foreign key
    new_frag.mmp_link = new_mmp
    # Get the size
